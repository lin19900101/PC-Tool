...
static unsigned long SMALL_PRIMES[] = { 3, 5, 7, 11 };
#define SMALL_PRIME_COUNT 4
static int ProbablePrime PROTO_LIST ((NN_DIGIT *, unsigned long));
static int SmallFactor PROTO_LIST ((NN_DIGIT *, unsigned long));
static int FermatTest PROTO_LIST ((NN_DIGIT *, unsigned long));
 ...
 */
int GeneratePrime 
(NN_DIGIT *a,NN_DIGIT  *b,NN_DIGIT  *c,NN_DIGIT *d,unsigned long digits,R_RANDOM_STRUCT *randomStruct)
{
  int status;
 ...
 */
static int ProbablePrime (NN_DIGIT *a,unsigned long aDigits)
{
...
/* Returns nonzero iff a has a prime factor in SMALL_PRIMES.
   ...
   Assumes aDigits < MAX_NN_DIGITS.
 */
static int SmallFactor (NN_DIGIT *a,unsigned long aDigits)
{
  int status;
  NN_DIGIT t[1];
  unsigned long i;
...
/* Returns nonzero iff a passes Fermat's test for witness 2.
   ...
   Assumes aDigits < MAX_NN_DIGITS.
 */
static int FermatTest (NN_DIGIT *a,unsigned long aDigits)
{
  int status;
  NN_DIGIT t[MAX_NN_DIGITS], u[MAX_NN_DIGITS];
