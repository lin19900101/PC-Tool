...
static int RSAFilter PROTO_LIST
  ((NN_DIGIT *, unsigned long, NN_DIGIT *, unsigned long));
...
R_RANDOM_STRUCT *randomStruct)                          /* random structure */
{
  NN_DIGIT d[MAX_NN_DIGITS], dP[MAX_NN_DIGITS], dQ[MAX_NN_DIGITS],
    ...
    v[MAX_NN_DIGITS];
  int status;
  unsigned long nDigits, pBits, pDigits, qBits;
...
NN_DIGIT *a,unsigned long aDigits,NN_DIGIT *b, unsigned long  bDigits)
{
  int status;
 ...
 */
static int RelativelyPrime (NN_DIGIT *a,unsigned long aDigits,NN_DIGIT *b,unsigned long bDigits)
{
  int status;
